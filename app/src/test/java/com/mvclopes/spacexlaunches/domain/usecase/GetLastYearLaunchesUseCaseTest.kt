...
@ExperimentalTime
class GetLastYearLaunchesUseCaseTest {

    private val repositoryMock = mockk<SpaceXRepository>()
    private val useCase = GetLastYearLaunchesUseCase(repositoryMock)
    ...
    fun `when repository returns success useCase should emit a list of domain launch`() = runBlocking {
        ...
        coEvery { repositoryMock.getLastYearLaunches() } returns flowOf(domainList)
    ...
    fun `when repository returns error useCase should emit exception`() = runBlocking {
        ...
        coEvery { repositoryMock.getLastYearLaunches() } returns flow { throw expectedThrowable }
